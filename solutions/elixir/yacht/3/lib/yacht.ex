defmodule Yacht do
  ...
  def score(category, dice) when category === :full_house do 
    ...
    cond do
      elem(tuple_dice, 0) === elem(tuple_dice, 1) && elem(tuple_dice, 2) === elem(tuple_dice, 3) && elem(tuple_dice, 3) === elem(tuple_dice, 4) && elem(tuple_dice, 0) !== elem(tuple_dice, 4) -> Enum.reduce(dice, 0, &+/2)
      elem(tuple_dice, 0) === elem(tuple_dice, 1) && elem(tuple_dice, 1) === elem(tuple_dice, 2) && elem(tuple_dice, 3) === elem(tuple_dice, 4) && elem(tuple_dice, 0) !== elem(tuple_dice, 4) -> Enum.reduce(dice, 0, &+/2)
      true -> 0
    end
  end
