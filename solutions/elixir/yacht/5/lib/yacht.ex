defmodule Yacht do
  ...
  def score(category, dice) when category === :ones, do: sum_only(dice, 1)
  def score(category, dice) when category === :twos, do: sum_only(dice, 2)
  def score(category, dice) when category === :threes, do: sum_only(dice, 3)
  def score(category, dice) when category === :fours, do: sum_only(dice, 4)
  def score(category, dice) when category === :fives, do: sum_only(dice, 5)
  def score(category, dice) when category === :sixes, do: sum_only(dice, 6)

  def sum_only(dice, digit) do
    Enum.reduce(
      dice, 
      0,
  ...
  def score(category, dice) when category === :yacht do 
    sorted = Enum.sort(dice)
    tuple_dice = List.to_tuple(sorted)
    if(elem(tuple_dice, 0) === elem(tuple_dice, 4), do: 50, else: 0)
  end
end
